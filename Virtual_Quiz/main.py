...
pathCSV = r"C:\Users\anila\OneDrive\Documents\OpeenCV_Automation\Virtual_Quiz\Mcqs.csv"
...
while True:
    ...
    if qNo < qTotal:
        ...
        if hands:
            ...
            cursor = lmList[8][:2]  # Ensure cursor is (x, y)
            length, info = detector.findDistance(lmList[8][:2], lmList[12][:2])  # Pass only (x, y)
            print(length)
            if length < 35:
